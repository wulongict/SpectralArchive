...
//  vector<int>* possibleCharges;
  ...
  sPrecursorIon(){
    ...
    mz=0;
    monoMZ=0;
//    possibleCharges=new vector<int>; // done
    charge=0;
  }
  sPrecursorIon(const sPrecursorIon& p){
    ...
    mz=p.mz;
    monoMZ=p.monoMZ;
    m_possiblecharges =         p.m_possiblecharges;
//    possibleCharges=new vector<int>; // done
//    for(size_t i=0;i<p.possibleCharges->size();i++) possibleCharges->push_back(p.possibleCharges->at(i)); //done
    charge=p.charge;
  }
  ~sPrecursorIon(){
//    delete possibleCharges; //done
  }
...
//      for(size_t i=0;i<p.possibleCharges->size();i++) possibleCharges->push_back(p.possibleCharges->at(i)); // done
  ...
  void clear(){
    ...
    mz=0;
    monoMZ=0;
//    possibleCharges->clear(); //done
    m_possiblecharges.clear();
    charge=0;
  }
} sPrecursorIon;
