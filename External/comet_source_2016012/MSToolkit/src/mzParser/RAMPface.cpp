...
void readHeader(RAMPFILE *pFI, ramp_fileoffset_t lScanIndex, struct ScanHeaderStruct *scanHeader){
  ...
  for(i=0;i<(unsigned int)scanHeader->precursorCount;i++){
    ...
    if(i==0){
      ...
      scanHeader->numPossibleCharges=(int)p.m_possiblecharges.size();
      for(k=0;k<scanHeader->numPossibleCharges;k++){
//        memcpy(&scanHeader->possibleCharges[k*4],&p.possibleCharges->at(k),sizeof(int));
        memcpy(&scanHeader->possibleCharges[k*4],&p.m_possiblecharges.at(k),sizeof(int));
        if(k==7){
      ...
      if( (j+32+p.m_possiblecharges.size()*4) > PRECURSORARRAY_LENGTH-1) {
      ...
      k=(int)p.m_possiblecharges.size();
      memcpy(&scanHeader->additionalPrecursors[j+=4],&k,sizeof(int));
      for(k=0;k<(int)p.m_possiblecharges.size();k++){ //done
        memcpy(&scanHeader->additionalPrecursors[j+=k*4],&p.m_possiblecharges.at(k),sizeof(int));
      }
    }
  }
