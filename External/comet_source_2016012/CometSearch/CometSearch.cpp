                                    ...
                                    bool *pbDuplFragment)
{
   ...
   while (iStartPos < _proteinInfo.iProteinSeqLength)
   {
      ...
      if (iLenPeptide<MAX_PEPTIDE_LEN)
      {
         ...
         if (iWhichQuery != -1)
         {
             // the following code is added  to generated peptide list ..., now it is not used.
//            // in case of protein digest, we export peptide here
//            for(int aaidx = iStartPos; aaidx <= iEndPos; aaidx ++)
//            {
//               cerr << szProteinSeq[aaidx];
//            }
//            cerr << "\t" << dCalcPepMass << "\t" << iStartPos << "\t" << iEndPos << "\t" << szProteinName << endl;
