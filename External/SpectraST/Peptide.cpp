...
Peptide::Peptide(string pep, int ch, string mspModStr) {
    ...
    if (!AAAverageMassTable || !modAverageMassTable || !AAMonoisotopicMassTable || !modMonoisotopicMassTable ||
        !modTokenTable ||
...
string Peptide::mspMods() {
    ...
    if (!nTermMod.empty()) {
        ss << '/' << Peptide::NTermPos /* + 1 */ << ',' << stripped[0] /* "n-term" */ << ','
           << nTermMod; // Peptide::NTermPos = -1
    }
    for (map<int, string>::iterator i = mods.begin(); i != mods.end(); i++) {
        ss << '/' << (*i).first /* + 1 */ << ',' << stripped[(*i).first] << ',' << (*i).second;
    }
    if (!cTermMod.empty()) {
        ss << '/' << Peptide::CTermPos << ',' << stripped[stripped.length() - 1] << ','
           << cTermMod; // Peptide::CTermPos = -2
    }
...
//  }
    ...
    if (nextAA == '-' || nextAA == ']' ||
        ((stripped[stripped.length() - 1] == 'K' || stripped[stripped.length() - 1] == 'R') && nextAA != 'P')) {
        NTT++;
    }
...
void Peptide::defaultTables() {
    ...
    (*AAAverageMassTable)['c'] =
            (*AAAverageMassTable)['h'] + (*AAAverageMassTable)['o']; // C-terminus. The hydroxyl group
...
// added by L. WU Sep 20
// http://www.bmrb.wisc.edu/metabolomics/mol_mass.php?formula=C5H7NO&subaction=Natural+Composition&updateIsoComp=1&12C=98.93&13C=1.07&14C=0&1H=99.9885&2H=0.0115&3H=0&14N=99.632&15N=0.368&16O=99.757&17O=0.038&18O=0.205
// Proline Residue formula: C5H7NO Mono: 97.0527638520, Average: 97.115180
// http://www.bmrb.wisc.edu/metabolomics/mol_mass.php?formula=C4H5NOS&subaction=Natural+Composition&updateIsoComp=1&12C=98.93&13C=1.07&14C=0&1H=99.9885&2H=0.0115&3H=0&14N=99.632&15N=0.368&16O=99.757&17O=0.038&18O=0.205
// Thioproline Residue formula: C4H5NOS Mono: 115.0091844778  Average: 115.153600

    (*modAverageMassTable)["Thioproline"] = 18.03842; //115.153600 - 97.115180; // phosphorylation
    ...
    (*AAMonoisotopicMassTable)['c'] =
            (*AAMonoisotopicMassTable)['h'] + (*AAMonoisotopicMassTable)['o']; // C-terminus. The hydroxyl group
...
// added by L. WU Sep 20
    (*modMonoisotopicMassTable)["Thioproline"] = 17.9564206258; // Thioproline = 115.0091844778 - 97.0527638520
...
// add by L. WU Sep 20
    (*modTokenTable)["P[115]"] = "Thioproline"; // Proline -> Thioproline
...
void Peptide::deleteTables() {
    ...
    if (AAMonoisotopicNeutralLossTable) {
        for (map<char, double *>::iterator i = AAMonoisotopicNeutralLossTable->begin();
             i != AAMonoisotopicNeutralLossTable->end(); i++) {
            if (i->second) {
                delete[] i->second;
            }
        }
    ...
    if (modMonoisotopicNeutralLossTable) {
        for (map<string, double *>::iterator i = modMonoisotopicNeutralLossTable->begin();
             i != modMonoisotopicNeutralLossTable->end(); i++) {
            if (i->second) {
                delete[] i->second;
            }
        }
    ...
    if (AAMonoisotopicImmoniumTable) {
        for (map<string, double *>::iterator i = AAMonoisotopicImmoniumTable->begin();
             i != AAMonoisotopicImmoniumTable->end(); i++) {
            if (i->second) {
                delete[] i->second;
            }
        }
...
void Peptide::generateFragmentIons(vector<FragmentIon *> &ions, string fragmentationType) {
    ...
    if (fragmentationType.empty() || fragmentationType == "CID" || fragmentationType == "CID-QTOF" ||
...
void Peptide::generateFragmentIonsCID(vector<FragmentIon *> &ions) {
    ...
    for (unsigned int ch = 1; ch <= (unsigned int) charge; ch++) {
        ...
        for (int i = (int) (stripped.length()) - 1; i >= 0; i--) {
            ...
            if (i > 0) {
                ...
                unsigned prom = 9;
                if (ch == charge && stripped[i] != 'P' && (double) position > (double) (stripped.length()) * 0.77)
                    prom = 6;
        ...
        for (int i = 0; i < (int) (stripped.length()) - 1; i++) {
            ...
            if (position == NAA() - 1) {
                ions.push_back(
                        new FragmentIon("b", position, -18, (sum + (*AAMonoisotopicMassTable)['!']) / (double) ch, ch,
                                        ch == charge ? 5 : 6));
            }

            // a ion
            // small a ions are more common
            ions.push_back(new FragmentIon("a", position, 0,
                                           (sum - (*AAMonoisotopicMassTable)['$'] - (*AAMonoisotopicMassTable)['o']) /
                                           (double) ch, ch, position <= 3 && ch == 1 ? 7 : 4));
    ...
    if (numP > 0) {
        ions.push_back(
                new FragmentIon("p", 0, 134, (precursorMH - phosphoMass - 3 * waterMass) / (double) charge, charge, 7));
    }
    ...
    if (numP > 1) {
        ...
        ions.push_back(
                new FragmentIon("p", 0, 178, (precursorMH - 2 * phosphoMass - waterMass) / (double) charge, charge, 7));
        ions.push_back(
                new FragmentIon("p", 0, 196, (precursorMH - 2 * phosphoMass - 2 * waterMass) / (double) charge, charge,
                                9));
        ions.push_back(
                new FragmentIon("p", 0, 214, (precursorMH - 2 * phosphoMass - 3 * waterMass) / (double) charge, charge,
                                7));
    }
...
bool Peptide::parseMspModStr(string mspModStr, bool add) {
    ...
    while ((m = nextToken(mspModStr, slashPos + 1, slashPos, "/ \t\r\n")) != "") {
        ...
        bool success = true;
        if (modPos == Peptide::NTermPos ||
            (modPos == 0 && isNTermModType(modType) && modType != "iTRAQ4plex" && modType != "iTRAQ4plexAcetyl" &&
             modType != "iTRAQ8plex" && modType != "iTRAQ8plex:13C(6)15N(2)")) {
...
Peptide *Peptide::shuffle(double minHomology, unsigned int numAAsToAdd) {
    ...
    for (unsigned int k = 0; k < numAAsToAdd; k++) {
        ...
        if (randIndex >= aas.length())
            randIndex--; // out-of-bound safeguard: apparently on some platform this could happen
...
Peptide *Peptide::shufflePeptideSequence(map<int, set<string> > &allSequences) {
    ...
    do {
        ...
        for (pos = 0; pos < numAA - 1; pos++) {
            ...
            unsigned int randIndex = (unsigned int) ((double) rand() / (double) RAND_MAX *
                                                     (double) (tmpMovableAA.size()));
            if (randIndex >= tmpMovableAA.size())
                randIndex--; // out-of-bound safeguard: apparently on some platform this could happen
            list<char>::iterator j = tmpMovableAA.begin();
            for (unsigned int i = 0; i < randIndex; i++) j++; // step forward movable AA indexed by randIndex
...
bool Peptide::processNewMod(char aa, double &deltaMass, string &modType, string &userToken) {
    ...
    if (foundToken != modTokenTable->end()) {
        ...
        if (!modType.empty() &&
            ((foundModType = modMonoisotopicMassTable->find(modType)) != modMonoisotopicMassTable->end())) {
...
void Peptide::permuteModTokens(vector<map<char, set<string> > > &allowableTokenSets,
...
FragmentIon::FragmentIon(string ionType, int pos, int loss, double mz, unsigned int ch, unsigned int prominence,
                         unsigned int isotope, double mzDiff, bool bracket) :
        m_ion(""),
        m_pos(pos),
        m_loss(loss),
        m_mz(mz),
