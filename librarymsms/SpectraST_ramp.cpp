            ...
            if ((indexOffset < 0) || (retryLoop == 0)) { // derive the index by inspection
                ...
                while ((nread = (int) ramp_fread(buf, sizeof(buf) - 1, pFI)) > taglen) {
                    ...
                    while (NULL != (find = strstr(look, scantag))) {
                        ...
                        if (reallocSize <= newN) {
                            reallocSize = newN + 500;
                            pScanIndex = (ramp_fileoffset_t *) realloc(pScanIndex,
                                                                       sizeof(ramp_fileoffset_t) * reallocSize);
                            if (!pScanIndex) {
                                printf("Cannot allocate memory\n");
                                return NULL;
                            }
        ...
        if (pFI->bIsMzData) {  // TODO
            ...
            while (!strstr(stringBuf, isAncient ? "</instrument" : "</msInstrument") &&
                ...
                if (!found[2]) {
                    ...
                    if (result != NULL &&
                        setTagValue(result, output->ionisation, INSTRUMENT_LENGTH,
                                    isAncient ? "ionisation=" : "value="))
                        found[2] = 1;
                }
                if (!found[3]) {
