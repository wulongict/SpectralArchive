...
//#include "../SpectraST/SpectraST_cramp.hpp"
...
//    std::cout << arg << " what" << what << std::endl;
        switch (what) {
            case RAMP_RUNINFO:
            case RAMP_HEADER:
            case RAMP_PEAKS:
...
//        std::cout << "scan offset is : " << scanOffset << std::endl;
            if (scanOffset >= 0) {
...
//                    std::cout << "m_handle : " << m_handle << " scan info ptr NOT ZERO " << returnPtr << std::endl;
                        if (returnPtr) {
