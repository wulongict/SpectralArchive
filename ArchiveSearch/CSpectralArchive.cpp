        ...
        if ( files.empty()){
            ...
            thread addingfiles([&](){
                while(true){
                    ...
                    spdlog::get("A")->info("time used {:.1f}s, {:.1f} file/min, {} spec/s ETA: {:.1f}s, -> {} hours {} minutes", time_elapsed, filenum_added/time_elapsed*60, spec_num_added/time_elapsed ,time_to_be_used, int(time_to_be_used)/3600, (int(time_to_be_used)%3600)/60 );
                    std::lock_guard<std::mutex> lock(m);
                    q.pop();
                }
            });
