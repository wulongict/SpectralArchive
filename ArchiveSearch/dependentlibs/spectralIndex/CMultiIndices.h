...
public:
	...
	void toGPU();
	void toCPU();
	void initialize(int numIndex, string indexpath, string basename, bool useMyOwn, shared_ptr<CPQParam> option);

	void add(float *vec, long specnum);
    void getANNByShuffleQueries(int numQuery, int ret_num,
                                const float *vquery, vector<vector<long>> &multipleInd,
...
public:
    CMultiIndices(string indexstring, string indexpath, string libname, bool doShuffle,
    ...
    void collectANNs(int indexChoice, int ret_num, const vector<vector<long>> &results, int idx,
...
private:
    ...
    void verifyEachQuery(DataFile &df, DataFile &splib, int indexChoice, int ret_num, PeptideProphetParser &ppp,
                         double probThreshold, Progress &ps, const vector<vector<long>> &results,
    ...
    void getAnnsForQueries(int numQuery, int ret_num, float *vquery, vector<vector<long>> &multipleInd,
                           bool verbose);
